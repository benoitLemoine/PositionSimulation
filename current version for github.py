...
# encounter IDs, currently unused, but keeping it here just in case while building out other functionality
...
def parse_position(report, start, end, boss_npcID, local_boss_ID):
    ...
    query = """
    {
        query($report: String!, $start: String!, $endTime: String!, %local_boss_ID: Int!) {
            reportData {
                report(code: %report) {
                      events(startTime: %start, endTime: %end, killType: Kills, hostilityType:Enemies, dataType: DamageTaken, limit: 10000, includeResources: true, targetInstanceID: %local_boss_ID) {
                        data 
                        nextPageTimestamp
                      }    
                }  
    ...
    variables = {
        ...
        "start": start,
        "end": end,
        "local_boss_ID": local_boss_ID
    }

    payload = json.dumps({"query": query, "variables": variables})
    conn.request("POST", "/api/v2", payload, token)
...
def get_start_end_EID(report,selection):
    ...
    for i in range (1,len(s1)):
        ...
        s2[i-1][3]=s2[i-1][3].strip(']')
        # I DO NOT KNOW WHY I NEED TO DO THIS TWICE, BUT IF I DONT, IT LEAVES AN EXTRA }
        s2[i-1][3]=s2[i-1][3].strip('}')
    for i in s2:
        if selection==i[0]:
            return [i[1],i[2],i[3]]
...
def TXY_to_TM(TXY,interval=.2500):
    """formats the Timestamps and the coordinates into a usable format of [time,total distance moved during that interval]"""
...
def movement_intervals(TM,move_threshold=3,Twindow=2 ):
    """
    Filters and groups time markers into moves
    args: move_threshold: the minimum distance the move must cover to be inclduded in the results (unit it yards)
    args: Twindow: time between moves before the movement is confirmed as having ended (unit is seconds)
    """
